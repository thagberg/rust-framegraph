...
impl<'d> CopyPassNode<'d> {
    pub fn builder(name: String) -> CopyPassNodeBuilder<'d> {
        CopyPassNodeBuilder {
            name,
            ..Default::default()
        }
...
impl<'d> CopyPassNodeBuilder<'d> {
    pub fn copy_src(mut self, copy_src: Arc<Mutex<DeviceResource<'d>>>) -> Self {
        self.copy_sources.push(copy_src);
        self
    }

    pub fn copy_dst(mut self, copy_dst: Arc<Mutex<DeviceResource<'d>>>) -> Self {
    ...
    pub fn build(mut self) -> Result<CopyPassNode<'d>, &'static str> {
